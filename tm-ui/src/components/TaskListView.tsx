...
export const TaskListView: React.FC<TaskListViewProps> = ({ tasks, onEditTask, onUpdateTask }) => {
    ...
    return (
        <div className="max-w-[1200px] mx-auto p-4 md:p-8 animate-enter">
                    ...
                    {tasks.length === 0 ? (
                        ...
                        tasks.map((task) => (
                            <div
                                key={task.id}
                                onClick={() => onEditTask(task)}
                                className="p-4 flex flex-col gap-3 active:bg-slate-50/80 dark:active:bg-slate-800/50 transition-all cursor-pointer"
                            >
                                <div className="flex justify-between items-start">
                                    <div className="flex flex-col gap-1">
                                        <div className={`px-2 py-0.5 rounded-full text-[9px] font-black inline-block border w-fit ${task.status === 'DONE' ? 'text-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 border-emerald-100 dark:border-emerald-900/30' :
                                            task.status === 'DOING' ? 'text-blue-500 bg-blue-50 dark:bg-blue-900/20 border-blue-100 dark:border-blue-900/30' :
                                                'text-slate-400 bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-white/5'
                                            }`}>
                                            {task.status === 'DONE' ? 'CONCLUÍDO' :
                                    ...
                                    <button
                                        type="button"
                                        onClick={(e) => handleStarToggle(e, task)}
                                        className={`p-1 rounded-full transition-all active:scale-95 ${task.important ? 'text-amber-500' : 'text-slate-200 dark:text-slate-700'}`}
                                    >
                                        <Star
                                            size={20}
                                            fill={task.important ? 'currentColor' : 'none'}
                                ...
                                <div className="flex justify-between items-center mt-1">
                                    ...
                                    <button
                                        type="button"
                                        onClick={(e) => handlePriorityToggle(e, task)}
                                        className={`px-2 py-0.5 rounded-lg text-[9px] font-black border tracking-wider shadow-sm ${getPriorityDetails(task.priority).color}`}
                                    >
                                        {getPriorityDetails(task.priority).label}
                                    </button>
                                </div>
